...
# FP8HYBRID=1: A is fp8e4m3, B is fp8e5m2
# FP8HYBRID=2: A is fp8e5m2, B is fp8e4m3
if getenv("FP8HYBRID") == 1: dtype_in, dtype_in_b = dtypes.fp8e4m3, dtypes.fp8e5m2
elif getenv("FP8HYBRID") == 2: dtype_in, dtype_in_b = dtypes.fp8e5m2, dtypes.fp8e4m3
else: dtype_in_b = None
...
tol_map = {dtypes.half:{1e-3, 1e-2}, dtypes.bfloat16:(1e-3, 1e-2), dtypes.fp8e4m3:(1e-1, 1e-1), dtypes.fp8e5m2:(1.0, 5e-1)}
atol, rtol = tol_map.get(dtype_in, (1e-4, 3e-2))
...
if __name__ == "__main__":
  ...
  a, b = init_matrix(M, K, dtype_in), init_matrix(K, N, dtype_in_b or dtype_in)
  for i in range(CNT):
