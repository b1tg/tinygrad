...
class WmmaUOpBuilder:
  ...
  def emit_smem_half2_epilogue(self, acc_regs, c_uop, grid_m, grid_n, wg_m, wg_n, wg_threads, threads, smem):
    ...
    out128_d_off = (
      grid_m * v.M_TILE * (self.N // 8) + grid_n * (v.N_TILE // 8) + (threads // 128) * 16 * (self.N // 8) +
      (threads // 16 % 8) * (self.N // 8) + threads % 16
    )
    ...
    for am in range(v.M_FRAGS):
      for half_idx, (gep0, gep1) in enumerate([(0, 1), (2, 3)]):
        ...
        for bn in range(v.N_FRAGS):
          ...
          h2_val = self.u(Ops.VECTORIZE, dtypes.half.vec(2), (
            self.u(Ops.GEP, dtypes.half, (c_vec,), (gep0,)),
            self.u(Ops.GEP, dtypes.half, (c_vec,), (gep1,)),
          ))
  ...
  def build(self) -> list[UOp]:
    ...
    # Global definitions
    c_uop = self.u(Ops.PARAM, out_dtype.ptr(M * N), (), 0)
    a_uop = self.u(Ops.PARAM, dtypes.half.ptr(M * K), (), 1)
    b_uop = self.u(Ops.PARAM, dtypes.half.ptr(K * N), (), 2)
    ...
    if variant.stages == 1:
      ...
      self.emit_prefetch(
        smem_a, smem_b, store_smem_a_off, store_smem_b_off, global_a_off_loop + block_k * variant.K_TILE,
        global_b_off_loop + block_k * variant.K_TILE * N, a_uop, b_uop
      )
      ...
      self.emit_prefetch(
        smem_a_curr, smem_b_curr, store_smem_a_off, store_smem_b_off, global_a_off_loop + block_k * variant.K_TILE,
        global_b_off_loop + block_k * variant.K_TILE * N, a_uop, b_uop
      )
      ...
      if_prefetch = self.u(Ops.IF, dtypes.void, (cond_prefetch,))
      self.emit_prefetch(
        smem_a_store, smem_b_store, store_smem_a_off, store_smem_b_off, global_a_off_loop + block_k * variant.K_TILE,
        global_b_off_loop + block_k * variant.K_TILE * N, a_uop, b_uop
      )
      self.u(Ops.END, dtypes.void, (if_prefetch,))
      self.cp_commit()
