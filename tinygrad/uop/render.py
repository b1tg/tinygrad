...
def pretty_print(x:UOp, cache=None, d=0, max_depth=100)->str:
  def dfs(x:UOp, cache:dict, seen:set[UOp]|None=None):
    if seen is None: seen = set()
    if x in seen: return
    seen.add(x)
    for s in x.src:
      cache.setdefault(s, [len(cache), 0, False])[1] += 1
      if cache[s][1] == 1: dfs(s, cache, seen)
  if cache is None: dfs(x, cache:={})
  if (cx:=cache.setdefault(x, [0,0,False]))[2]: return f"{' '*d}x{cx[0]}"
  if d >= max_depth: return f"{' '*d}x{cx[0]}:=..."
