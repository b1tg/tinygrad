...
class CompiledRunner(Runner):
  def __init__(self, p:ProgramSpec, prg=None):
    ...
    self._prg = Device[p.device].runtime(p.function_name, self.p.lib, *p.aux, runtimevars=p.runtimevars,
                                          var_fmts=tuple('Q' if v.dtype.itemsize > 4 else 'I' for v in p.vars)) if prg is None else prg
