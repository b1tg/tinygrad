...
class TensorCore: # D = A * B + C, A is (M x K), B is (K x N), C and D are (M x N)
  ...
  dtype_a: DType # dtype for A
  dtype_b: DType # dtype for B
  ...
  def __str__(self):
    dtype_str = self.dtype_a.name if self.dtype_a == self.dtype_b else f"{self.dtype_a.name}_{self.dtype_b.name}"
...
cuda_81616 = [TensorCore(dims=(8,16,16), threads=32, elements_per_thread=(8,4,4), dtype_a=di, dtype_b=di, dtype_out=do, opts=cuda_tc_opts,
...
cuda_81632_f8 = [TensorCore(dims=(8,16,32), threads=32, elements_per_thread=(16,8,4), dtype_a=dtype_a, dtype_b=dtype_b, dtype_out=do, opts=cuda_tc_opts,
...
cuda_8168_tf32 = [TensorCore(dims=(8,16,8), threads=32, elements_per_thread=(4,2,4), dtype_a=dtypes.float, dtype_b=dtypes.float, dtype_out=dtypes.float, opts=cuda_tc_opts,
...
amd_rdna3 = [TensorCore(dims=(16,16,16), threads=32, elements_per_thread=(16,16,8), dtype_a=di, dtype_b=di, dtype_out=do,
...
amd_rdna4 = [TensorCore(dims=(16,16,16), threads=32, elements_per_thread=(8,8,8), dtype_a=di, dtype_b=di, dtype_out=do,
...
amd_cdna_161616 = [TensorCore(dims=(16,16,16), threads=64, elements_per_thread=(4,4,4), dtype_a=di, dtype_b=di, dtype_out=do,
...
amd_cdna_161632 = [TensorCore(dims=(16,16,32), threads=64, elements_per_thread=(8,8,4), dtype_a=di, dtype_b=di, dtype_out=do,
  opts=("l0","l0","l0","l0","u1","u1","l1","l1"),
...
metal = [TensorCore(dims=(8,8,8), threads=32, elements_per_thread=(2,2,2), dtype_a=di, dtype_b=di, dtype_out=do,
...
amx = [TensorCore(dims=(sz,sz,1), threads=1, elements_per_thread=(sz,sz,sz*sz), dtype_a=dt, dtype_b=dt, dtype_out=dt,
...
intel = [TensorCore(dims=(8,8,16), threads=8, elements_per_thread=(16,16,8), dtype_a=dtypes.half, dtype_b=dtypes.half, dtype_out=dtypes.float,
