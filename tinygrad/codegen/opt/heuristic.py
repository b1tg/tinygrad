  ...
  if k.ren.has_local and getenv("MV",1) != 0 and (MV_BLOCKSIZE > 1 or MV_THREADS_PER_ROW > 1 or MV_ROWS_PER_THREAD > 1) and  \
    ...
    if mulop.op is Ops.MUL:
      ...
      if src0.op is Ops.INDEX or src1.op is Ops.INDEX:
        # orient so idx_mat is the INDEX side (weight matrix) and find the other side's reduce range usage
        if src1.op is Ops.INDEX and src0.op is not Ops.INDEX: src0, src1 = src1, src0
        idx0, idx1 = src0.src[1].get_idx(), src1.src[1].get_idx() if src1.op is Ops.INDEX else None
        if k.ranges_of(AxisType.REDUCE):
          first_reduce_rng = k.ranges_of(AxisType.REDUCE)[0]
          # for two INDEX: original check. for one INDEX: just check reduce range is in the INDEX side
          check_pass = (idx1 is not None and any(u is first_reduce_rng for u in idx0.split_uop(Ops.ADD))
                        and all(r in idx1.ranges for r in idx0.ranges)) or \
                       (idx1 is None and any(u is first_reduce_rng for u in idx0.split_uop(Ops.ADD)))
          if check_pass:
