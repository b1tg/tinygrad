...
from tinygrad.runtime.support.compiler_amd import AMDHIPCompiler, AMDLLVMCompiler
...
class AMDDevice(HCQCompiled):
  ...
  def __init__(self, device:str=""):
    ...
    super().__init__(device, AMDAllocator(self), AMDLLVMRenderer() if getenv("AMD_LLVM", 0) else AMDRenderer(self.arch),
                     AMDLLVMCompiler(self.arch) if getenv("AMD_LLVM", 0) else AMDHIPCompiler(self.arch), functools.partial(AMDProgram, self),
