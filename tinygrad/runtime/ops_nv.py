...
class NVComputeQueue(NVCommandQueue):
  ...
  def exec(self, prg:NVProgram, args_state:NVArgsState, global_size:tuple[sint, ...], local_size:tuple[sint, ...]):
    ...
    print(f"==global_size, local_size: {global_size}, {local_size}")
    print(f"==prg.constbufs: {prg.constbufs}, {prg.constbufs[0][1]}")
    hexdump.hexdump(to_mv(mv_address(prg.qmd), 0x100))
...
class NVProgram(HCQProgram):
  def __init__(self, dev:NVDevice, name:str, lib:bytes):
    ...
    print(f"== self.prog_addr: 0x{self.prog_addr:x}, self.prog_addr>>8: 0x{self.prog_addr>>8:x}")
    print(f"== self.prog_addr>>40: 0x{self.prog_addr>>40:x}")
    print(f"== smem_cfg: 0x{smem_cfg:x}, self.shmem_usage: 0x{self.shmem_usage:x}")
    ...
    for i,(addr,sz) in self.constbufs.items():
      print(f"== const buf: i=0x{i:0x}, addr=0x{addr:x}, sz=0x{sz:x}")      
    ...
    bw_qmd = [
      ...
      0x00, 0x00, 0x3F, 0x01, 0x00, 0x00, 0x00, 0x00,  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00,  0x00, 0x00, 0x19, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x81, 0x07, 0x02, 0x00, 0x00, 0x00, 0x00,  0xA4, 0x03, 0x50, 0x2F, 0x3C, 0x7F, 0x22, 0x07,
      # 0x040
      0x02, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00,  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x80,
      0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      # 0x080
      # NVCEC0_QMDV05_00_PROGRAM_ADDRESS_LOWER
      # addr_low              # addr_upper
      0xB0, 0x56, 0xF7, 0xFD, 0xF3, 0x07, 0x40, 0x01,  0x03, 0x00, 0x01, 0x00, 0x01, 0x10, 0x02, 0x00,
      0x08, 0x48, 0xB4, 0x04, 0x00, 0x00, 0x64, 0x00,  0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,
      0x01, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,  0x00, 0xA0, 0x78, 0xFF, 0xFC, 0x01, 0x00, 0x02,
      0x00, 0x04, 0x78, 0xFF, 0xFC, 0x01, 0x80, 0x04,  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      # 0x0C0
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x78, 0xFF, 0xFC, 0x01, 0x00, 0x80,  0x11, 0x00, 0x00, 0x10, 0x6B, 0x75, 0xDF, 0x3F,
...
class NVDevice(HCQCompiled[NVSignal]):
  ...
  def __init__(self, device:str=""):
    ...
    self.usermode = rm_alloc(self.fd_ctl, nv_gpu.BLACKWELL_USERMODE_A, self.root, self.subdevice, None).hObjectNew
  ...
  def _new_gpu_fifo(self, gpfifo_area, ctxshare, channel_group, offset=0, entries=0x400, enable_debug=False) -> GPFifo:
    ...
    return GPFifo(ring=to_mv(gpfifo_area.va_addr + offset, entries * 8).cast("Q"), entries_count=entries, token=ws_token_params.workSubmitToken,
                  controls=nv_gpu.BlackwellBControlGPFifo.from_address(gpfifo_area.va_addr + offset + entries * 8))
