...
def ggml_data_to_tensor(t: Tensor, n: int, ggml_type: int) -> Tensor:
  ...
  if (nelements_nbytes := { 2: (32, 18), 3: (32, 20), 8: (32, 34), 12: (256, 144), 13: (256, 176), 14: (256, 210), 39: (32, 17) }.get(ggml_type)) is not None:
    ...
    if ggml_type == 13:  # Q5_K: 256 elements per 176-byte block (d:2, dmin:2, scales:12, qh:32, qs:128)
      ...
      sc = s[:,0:4].bitwise_and(63).cat(s[:,8:12].bitwise_and(0xF).bitwise_or(s[:,0:4].rshift(6).lshift(4)), dim=-1)
      mn = s[:,4:8].bitwise_and(63).cat(s[:,8:12].rshift(4).bitwise_or(s[:,4:8].rshift(6).lshift(4)), dim=-1)
      qh = blocks[:,16:48].reshape(-1,1,32).expand((-1,4,32))
      qs = blocks[:,48:176].reshape(-1,4,32)
      m1 = Tensor([1,4,16,64], dtype=dtypes.uint8, device=t.device).reshape(1,4,1)
      m2 = Tensor([2,8,32,128], dtype=dtypes.uint8, device=t.device).reshape(1,4,1)
      ql = qs.bitwise_and(0xF).cast(dtypes.float32) + qh.bitwise_and(m1).idiv(m1).cast(dtypes.float32) * 16
      qh = qs.rshift(4).cast(dtypes.float32) + qh.bitwise_and(m2).idiv(m2).cast(dtypes.float32) * 16
      q = Tensor.stack(ql, qh, dim=2).reshape(-1,8,32)
      return (d * sc.unsqueeze(-1) * q - dmin * mn.unsqueeze(-1)).flatten(-2)
    if ggml_type == 14:
