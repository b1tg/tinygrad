...
def ggml_data_to_tensor(t: Tensor, n: int, ggml_type: int) -> Tensor:
  ...
  Supported quantized types: Q4_0 (id: 2), Q4_1 (id: 3), Q5_0 (id: 6), Q5_1 (id: 7), Q8_0 (id: 8),
                             Q4_K (id: 12), Q6_K (id: 14), MXFP4 (id: 39)
  """
  ...
  if (nelements_nbytes := { 2: (32, 18), 3: (32, 20), 6: (32, 22), 7: (32, 24), 8: (32, 34), 12: (256, 144), 14: (256, 210), 39: (32, 17) }.get(ggml_type)) is not None:
    ...
    if ggml_type == 6:
      ...
      qh = q_to_uint8(blocks[:, 2:6], 1).reshape(-1, 8, 4).permute((0, 2, 1)).reshape(-1, 32).cast(dtypes.uint8)
      ql = q_to_uint8(blocks[:, 6:22], 4).cast(dtypes.uint8)
      qs = ql.bitwise_or(qh.lshift(4)).cast(dtypes.float32) - 16.0
      return d * qs
    if ggml_type == 7:
      d, m = (blocks[:,s:s+2].bitcast(dtypes.float16).cast(dtypes.float32) for s in [ 0, 2 ])
      qh = q_to_uint8(blocks[:, 4:8], 1).reshape(-1, 8, 4).permute((0, 2, 1)).reshape(-1, 32).cast(dtypes.uint8)
      ql = q_to_uint8(blocks[:, 8:24], 4).cast(dtypes.uint8)
      qs = ql.bitwise_or(qh.lshift(4)).cast(dtypes.float32)
      return d * qs + m
