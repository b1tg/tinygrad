...
def ggml_data_to_tensor(t: Tensor, n: int, ggml_type: int) -> Tensor:
  ...
  if (nelements_nbytes := {
    2:(32,18), 3:(32,20), 6:(32,22), 8:(32,34), 12:(256,144), 13:(256,176), 14:(256,210), 39:(32,17)
    ...
    if ggml_type == 6:  # Q5_0: 32 elements per 22-byte block (d:2, qh:4, qs:16)
      ...
      qh = blocks[:,2:6]  # 4 bytes = 32 high bits
      ql = q_to_uint8(blocks[:,6:], 4)  # 32 low 4-bit values
      # extract 32 bits from qh without transposing (sequential bit order within each byte)
      high_bits = qh.unsqueeze(-1).expand(*qh.shape, 8).idiv(
        Tensor.stack(*[Tensor(2**i, device=t.device, dtype=qh.dtype) for i in range(8)])).bitwise_and(1).reshape(-1, 32).cast(dtypes.uint8).lshift(4)
      return (ql.bitwise_or(high_bits).bitcast(dtypes.int8) - 16) * d
