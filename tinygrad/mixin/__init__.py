...
class OpMixin(ElementwiseMixin, ReduceMixin):
  ...
  def _getitem(self, indices, v=None) -> Self:
    ...
    if v is None and isinstance(indices, type(self)) and isinstance(self.device, tuple) and self._uop.axis not in (None, 0):
      branches = tuple(self._wrap_uop(self._uop.mselect(i))[indices.to(d)]._uop for i,d in enumerate(self.device))
      return self._wrap_uop(UOp(Ops.MSTACK, self.dtype, branches).multi(self._uop.axis - 1 + len(indices.shape)))
