...
class OpMixin(ElementwiseMixin, ReduceMixin):
  ...
  def arange(cls, start, stop=None, step=1, dtype:DTypeLike|None=None) -> Self:
    ...
    # symbolic length needs the O(n) two-stage cumsum; concrete folds at trace time either way, so leave it on _cumalu (no change to the common path)
  ...
  def _split_cumalu(self, axis:int, op:Ops) -> Self:
    ...
    if isinstance(s:=self.shape[axis], int) and s <= SPLIT*2: return self._cumalu(axis, op)  # symbolic always uses the two-stage (O(n)) path
    # pad on the back so the result is recovered with the front slice [:s] (in-bounds for symbolic s; the back slice [-s:] isn't)
