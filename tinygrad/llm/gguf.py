...
def _gguf_parse(tensor: Tensor, device_fn: Callable[[str], str|None]|None = None) -> tuple[dict, dict[str, Tensor]]:
  ...
  for name, dims, typ, off in t_infos:
    if (dev := device_fn(name)) is None: continue
    out[name] = ggml_data_to_tensor(tensor[data_start+off:data_start+ends[off]].to(dev).realize(), prod(dims), typ).reshape(*reversed(dims))
  return kv_data, out
...
def gguf_load(src: Tensor|str|pathlib.Path, device_fn: Callable[[str], str|None]|None = None) -> tuple[dict, dict[str, Tensor]]:
  ...
  returning None from `device_fn` skips that tensor entirely (no dequant, no device copy).
  ...
  for pp in _gguf_split_paths(pathlib.Path(src), kv['split.count'])[1:]: sd.update(load(pp)[1])
  return kv, sd
