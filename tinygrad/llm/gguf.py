...
GGUFLoader = Callable[[dict, str, Tensor, tuple[int, ...], int], Tensor|GGUFQuantizedTensor]

def _gguf_parse(tensor:Tensor, loader:GGUFLoader|None=None) -> tuple[dict, dict[str, Tensor|GGUFQuantizedTensor]]:
  if loader is None: tensor = tensor.to(None).realize()
  ...
  for name, dims, typ, off in t_infos:
    n = prod(dims)
    if typ not in _GGML_NATIVE and typ not in _GGML_QUANT: raise ValueError(f"unsupported GGML type {typ}")
    if typ in _GGML_QUANT and n % _GGML_QUANT[typ][0]: raise ValueError(f"{name}: incomplete GGML block")
    size = n*_GGML_NATIVE[typ].itemsize if typ in _GGML_NATIVE else n//_GGML_QUANT[typ][0]*_GGML_QUANT[typ][1]
    if data_start+off+size > tensor.numel(): raise ValueError(f"{name}: tensor extends past end of GGUF")
    raw = tensor[data_start+off:data_start+off+size]
    shape = tuple(reversed(dims))
    state_dict[name] = loader(kv_data, name, raw, shape, typ) if loader else ggml_data_to_tensor(raw, n, typ).reshape(shape)
...
def gguf_load(fn:Tensor|str|pathlib.Path, loader:GGUFLoader|None=None) -> tuple[dict, dict[str, Tensor|GGUFQuantizedTensor]]:
  """
  Loads a .gguf file, returning the `kv_data` and `state_dict`. Multi-part splits are auto-merged when loaded by path.
  A loader callback receives metadata, name, packed bytes, logical shape, and GGML type; payloads stay file-backed until used.
  ...
  """
  kv, sd = _gguf_parse(fn if isinstance(fn, Tensor) else Tensor(pathlib.Path(fn)), loader)
  ...
  for pp in _gguf_split_paths(pathlib.Path(fn), kv)[1:]: sd.update(_gguf_parse(Tensor(pp), loader)[1])
  return kv, sd
