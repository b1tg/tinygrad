...
def gguf_load_file(path: str|pathlib.Path) -> tuple[dict, dict[str, Tensor], int]:
  """Load a GGUF from a file path. Multi-part splits are auto-merged."""
  path = pathlib.Path(path)
  # TODO: remove the need for copy to default device
  kv, sd = gguf_load(Tensor(path).to(None).realize())
  nbytes = os.path.getsize(path)
  for pp in _gguf_split_paths(path, kv)[1:]:
    sd.update(gguf_load(Tensor(pp).to(None).realize())[1])
