...
class Transformer:
  ...
  @staticmethod
  def from_gguf(gguf:Tensor, max_context:int|None=None, realize=bool(getenv("REALIZE", 0)), shard:int=1) -> tuple[Transformer, dict]:
    if shard > 1:
      from tinygrad.device import Device
      devs = tuple(f"{Device.DEFAULT.split(':')[0]}:{i}" for i in range(shard))
      # peek header once to learn block_count, then route blk.N.* layer-wise (everything else to devs[0])
