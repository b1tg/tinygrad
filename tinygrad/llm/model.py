...
class TransformerBlock(FFNBlock):
  ...
  def _attention(self, x:Tensor, start_pos:int|UOp) -> Tensor:
    ...
    if getenv("FULL_KV"):
      ...
      mask = Tensor.full((1, 1, T, self.config.max_context), float("-inf"), dtype=x.dtype, device=x.device).triu(start_pos+1)
    else:
...
class MLATransformerBlock(FFNBlock):
  ...
  def _attention(self, x:Tensor, start_pos:int|UOp) -> Tensor:
    ...
    assigned_k = Tensor(self.cache_k.uop.after(self.cache_k[:, :, start_pos:start_pos+T, :].uop.store(k_store.uop)))
    if getenv("FULL_KV"):
      k = assigned_k
      v = k[..., :self.config.kv_lora_rank]
      mask = Tensor.full((1, 1, T, self.config.max_context), float("-inf"), dtype=x.dtype, device=x.device).triu(start_pos+1)
    else:
      k = assigned_k[:, :, 0:start_pos+T, :]
      v = k[..., :self.config.kv_lora_rank]
      mask = Tensor.full((1, 1, T, start_pos+T), float("-inf"), dtype=x.dtype, device=x.device).triu(start_pos+1) if resolve(T != 1) else None
