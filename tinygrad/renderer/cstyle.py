...
def _dot4_lane(x:UOp) -> tuple[UOp, int]|None:
  ...
  if x.dtype in dtypes.int32s: return x, lane
  return None
...
def _dot4_match_terms(ts:list[UOp]) -> tuple[UOp, UOp, UOp|None]|None:
  ...
  for muls, acc in candidates:
    lanes: dict[int, tuple[UOp, UOp]] = {}
    for t in muls:
  ...
  for terms, acc in candidates:
    lanes: dict[int, UOp] = {}
    for t in terms:
      if (a:=_dot4_lane(t)) is None or a[1] in lanes: break
      lanes[a[1]] = a[0]
    else:
      if set(lanes) == {0, 1, 2, 3} and all(lanes[s] is lanes[0] for s in (1, 2, 3)): return lanes[0], UOp.const(dtypes.int, 0x01010101), acc
  return None
...
def _render_dot4(ctx, x:UOp) -> str|None:
  def render(u): return hex(u.arg) if u.op is Ops.CONST and isinstance(u.arg, int) else ctx[u]
  def dot4(a, b, acc):
    ...
    return None
  if (m:=_dot4_match_terms(_dot4_terms(x))) is not None: return dot4(render(m[0]), render(m[1]), 0 if m[2] is None else ctx[m[2]])
  for dot, acc in ((x.src[0], x.src[1]), (x.src[1], x.src[0])):
    if (m:=_dot4_match_terms(_dot4_terms(dot))) is not None and m[2] is None and acc.dtype == dtypes.int:
      return dot4(render(m[0]), render(m[1]), ctx[acc])
  return None
