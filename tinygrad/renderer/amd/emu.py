...
# Wave size: CDNA uses 64-lane waves, RDNA uses 32-lane waves
...
SGPR_COUNT = 260
# Maximum VGPR buffer size (for CDNA with 64 lanes)
MAX_VGPR_SIZE = 256 * 64
...
def _lane_active(exec_mask: UOp, lane: UOp) -> UOp:
  """Check if a lane is active in the exec mask. Handles both 32 and 64-bit masks."""
  return ((exec_mask >> lane.cast(dtypes.uint64)) & _c(1, dtypes.uint64)).ne(_c(0, dtypes.uint64))
...
class _Ctx:
  ...
  def range(self, n: int = 64) -> UOp:
    """Create a lane range UOp with unique axis ID. Defaults to 64 for CDNA compatibility."""
    self._axis_id += 1
  ...
  def unroll_lanes(self, get_lane_bit, exec_mask: UOp, apply_exec: bool = True) -> UOp:
    """Combine lane bits into a mask using RANGE+REDUCE. Supports both 32 and 64 lane waves."""
    lane = self.range()
  ...
  def rvgpr_dyn(self, reg: UOp, lane: UOp, valid: UOp | None = None) -> UOp:
    ...
    idx = reg.cast(dtypes.int) * _c(64, dtypes.int) + lane.cast(dtypes.int)
...
def _compile_sopp(inst: ir3.SOPP | ir4.SOPP, ctx: _Ctx) -> UOp:
  ...
  if inst.op in _get_pcode_dict(inst.op):
    ...
    srcs = {'PC': pc_bytes.cast(dtypes.int64), 'SIMM16': simm16, 'SCC': ctx.rsgpr_dyn(_c(SCC.offset)), 'VCC': vcc,
            'VCCZ': vcc.eq(UOp.const(dtypes.uint32, 0)).cast(dtypes.uint32), 'EXECZ': exec_lo.eq(UOp.const(dtypes.uint64, 0)).cast(dtypes.uint32)}
...
class WaveState:
  ...
  def __init__(self, n_lanes: int, wave_size: int = 32):
    ...
    # Set EXEC mask: for n_lanes <= 32, only EXEC_LO is set; for n_lanes > 32, both EXEC_LO and EXEC_HI are set
...
def run_asm(lib: int, lib_sz: int, gx: int, gy: int, gz: int, lx: int, ly: int, lz: int, args_ptr: int, rsrc2: int = 0x19c,
  ...
  # CDNA uses 64-lane waves, RDNA uses 32-lane waves
  wave_size = 64 if arch == "cdna" else 32
