...
class TensorCore: # D = A * B + C, A is (M x K), B is (K x N), C and D are (M x N)
  ...
  def __post_init__(self):
    assert len(self.dtype_in) == 2 and self.dtype_in[0].itemsize == self.dtype_in[1].itemsize
...
def mix(a:DType, b:DType): return [(a,a), (b,b), (a,b), (b,a)]

# ***** NVIDIA *****

# https://docs.nvidia.com/cuda/parallel-thread-execution/#warp-level-matrix-instructions-for-mma
def mma(K:int, di:tuple[DType, DType], do:DType) -> TensorCore:
  ...
  k, g = [f"k{i}" for i in range(int(math.log2(K)))], int(math.log2(4//di[0].itemsize))
...
cuda_81616 = [mma(16,(di,di),do) for di,do in [(dtypes.half,dtypes.float),(dtypes.bfloat16,dtypes.float),(dtypes.half,dtypes.half)]]
cuda_81632_f8 = [mma(32,di,dtypes.float) for di in mix(dtypes.fp8e4m3,dtypes.fp8e5m2)]
cuda_8168_f16 = [mma(8,(dtypes.half,dtypes.half),do) for do in [dtypes.float, dtypes.half]]
cuda_8168_tf32 = [mma(8,(dtypes.float,dtypes.float),dtypes.float)]
...
# (16,16,16)
amd_rdna3 = [TensorCore(dtype_in=(di,di), dtype_out=do, frag_a=(("m0", "m1", "m2", "m3", "n0"), ("k0", "k1", "k2", "k3")),
...
# (16,16,16)
amd_rdna4 = [TensorCore(dtype_in=(di,di), dtype_out=do, frag_a=(("m0", "m1", "m2", "m3", "k2"), ("k0", "k1", "k3")),
...
def mfma(K:int, di:tuple[DType, DType], do:DType) -> TensorCore:
...
amd_cdna_161616 = [mfma(16,(di,di),dtypes.float) for di in [dtypes.half, dtypes.bfloat16]]
amd_cdna_161632 = [mfma(32,di,dtypes.float) for di in mix(dtypes.fp8e5m2,dtypes.fp8e4m3)+[(dtypes.half,dtypes.half),
                                                                                         (dtypes.bfloat16,dtypes.bfloat16)]]
amd_cdna_1616128 = [mfma(128,di,dtypes.float) for di in mix(dtypes.fp8e5m2,dtypes.fp8e4m3)]
amd_cdna3_161632 = [mfma(32,di,dtypes.float) for di in mix(dtypes.fp8e5m2fnuz,dtypes.fp8e4m3fnuz)]
...
# (8,8,8)
metal = [TensorCore(dtype_in=(di,di), dtype_out=do, frag_a=(("k1", "m0", "m1", "k2", "m2"), ("k0",)),
