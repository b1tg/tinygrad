...
class ShapeTracker:
  ...
  def flip1(self, mul: tuple[int, ...]) -> ShapeTracker: return ShapeTracker(self.views[0:-1] + (self.views[-1].flip1(mul), ))
  def stride(self, arg: tuple[int, ...]) -> ShapeTracker:
    steps = tuple(abs(x) for x in arg)
    arg = (-1 if x < 0 else 1 for x in arg)
    return self.flip((x == -1 for x in arg)).flip1(steps)
