...
multi_pm = PatternMatcher([
  ...
  (UPat(Ops.CUSTOM_KERNEL, src=UPat((Ops.MULTI, Ops.CONTIGUOUS, Ops.BUFFER)), name="ck"),
    lambda ck: ck.replace(src=tuple(m.src[0] if m.op in (Ops.MULTI,) else m for m in ck.src))),
