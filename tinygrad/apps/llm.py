...
class TransformerBlock:
  ...
  def _attention(self, x:Tensor, start_pos:int|UOp) -> Tensor:
    ...
    attn = q.scaled_dot_product_attention(k, v, attn_mask=mask, enable_gqa=True,
      attn_sink=self.attn_sinks.reshape(1, self.n_heads, 1, 1) if hasattr(self, "attn_sinks") else None)     # (B,H,T,Hd)
  ...
  def from_gguf(gguf:Tensor, max_context:int|None=None, realize=bool(getenv("REALIZE", 0))) -> tuple[Transformer, dict]:
    ...
    if arch == 'gpt-oss':
      for name in list(state_dict.keys()):
        ...
        if '.attn_sinks.weight' in name: state_dict[name.replace('.attn_sinks.weight', '.attn_sinks')] = state_dict.pop(name)
