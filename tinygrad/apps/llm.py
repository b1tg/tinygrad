...
class Transformer:
  ...
  def from_gguf(gguf:Tensor, max_context:int|None=None) -> tuple[Transformer, dict]:
    ...
    state_dict = {k:v.cast('fp8e4m3') if getenv("HALF", 1) else v for k,v in state_dict.items()}
...
if __name__ == "__main__":
  ...
  while 1:
    start_pos = len(ids) - 1
    try:
      ids += tok.role("user") + tok.encode("One of the first programming languages to provide single- and double-precision floating-point data types wa\
s Fortran. Before the widespread adoption of IEEE 754-1985, the representation and" + " and"*17) + [eos_id] + tok.role("assistant")
    except EOFError:
      break
    print(f"ids: {len(ids)}")
    assert len(ids)%32 == 0
