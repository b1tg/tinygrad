...
MODELS = {
  ...
  "32B-FP8": {
    "repo": "RedHatAI/QwQ-32B-FP8-dynamic",
    "model_params": {"dim": 5120, "n_heads": 40, "n_kv_heads": 8, "n_layers": 64, "norm_eps": 1e-5, "rope_theta": 1000000, "vocab_size": 152064, "hidden_dim": 27648},
    "total_num_weights": 7,
    "tokenizer": "RedHatAI/QwQ-32B-FP8-dynamic"
  },
}
...
class FP8Linear:
  def __init__(self, in_features, out_features, bias=True):
    ...
    if bias:
      self.bias = Tensor.ones(out_features, dtype=dtypes.bfloat16)
    else:
      self.bias = None
    self.weight_scale = Tensor.ones((out_features, 1), dtype=dtypes.float32)
    self.input_scale = Tensor.ones((out_features, 1), dtype=dtypes.float32)
  def __call__(self, x):
    old_type = x.dtype
    # self.weight.shape=(5120, 5120), x.shape=(1, 2, 5120), self.weight_scale.shape=(5120, 1)
    # print(f"{self.weight.shape=}, {x.shape=}, {self.weight_scale.shape=}")
    y = x.dot(self.weight.T.cast(x.dtype)*self.weight_scale.T) 
    # scale =  self.weight_scale.T  # [batch, seq, out_features]
    # y = res * scale

    if self.bias is not None:
      y = y + self.bias.cast(y.dtype)
    return y.cast(old_type)
    # return res.cast(old_type) + self.bias
...
def download_weights(total_num_weights:int, repo:str) -> Path:
  model = fetch(f"https://huggingface.co/{repo}/resolve/main/model.safetensors.index.json?download=true", "model.safetensors.index.json", subdir=(subdir:=repo.split("/")[-1]))

  for i in range(1, total_num_weights + 1):
    filename = f"model-{i:05d}-of-{total_num_weights:05d}.safetensors"
    fetch(f"https://huggingface.co/{repo}/resolve/main/{filename}?download=true", filename, subdir=subdir)

  return Path(os.path.dirname(model))

def load_model(model_path:Path, model_params:Dict[str, Union[int, float]]) -> Transformer:
  # build model
  model = Transformer(**model_params, linear=FP8Linear)

  # update layers to add bias
  updated_layers = []
  for layer in model.layers:
    head_dim = model_params["dim"] // model_params["n_heads"]
    layer.attention.wq = FP8Linear(model_params["dim"], model_params["n_heads"] * head_dim, bias=True)
    layer.attention.wk = FP8Linear(model_params["dim"], model_params["n_kv_heads"] * head_dim, bias=True)
    layer.attention.wv = FP8Linear(model_params["dim"], model_params["n_kv_heads"] * head_dim, bias=True)
  ...
  weights = convert_from_huggingface(load(str(model_path / "model.safetensors.index.json")), model_params["n_layers"], model_params["n_heads"], model_params["n_kv_heads"], permute_layers=False)
  # weights = FP8Linear.quantize(weights, None, dtypes.float16, False)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--size", choices=["32B", "32B-FP8"], default="32B-FP8", help="Model size")
  ...
  model_path = Path(args.weights) if args.weights else download_weights(model_info["total_num_weights"], model_info["repo"])
