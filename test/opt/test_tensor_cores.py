...
class TestTensorCores(unittest.TestCase):
  ...
  def test_tensor_cores(self):
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      if not is_dtype_supported(tc.dtype_a) or not is_dtype_supported(tc.dtype_b) or not is_dtype_supported(tc.dtype_out): continue
      # for AMX, tc.dims[2] == 1 so reduceop is None thus tensor_cores are not triggered
      helper_tc_allclose(tc.dims[0], tc.dims[1], 2 if AMX else tc.dims[2], (tc.dtype_a, tc.dtype_b), tc.dtype_out, axis=0, tc_opt=0)
  ...
  def test_tensor_cores_codegen(self):
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      ...
      a, b = Tensor.rand(m, k, dtype=tc.dtype_a), Tensor.rand(k, n, dtype=tc.dtype_b)
  ...
  def test_tensor_cores_padded(self):
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      if not is_dtype_supported(tc.dtype_a) or not is_dtype_supported(tc.dtype_b) or not is_dtype_supported(tc.dtype_out): continue
      helper_tc_allclose(tc.dims[0]+(pad:=1), tc.dims[1]+pad, tc.dims[2]+pad, (tc.dtype_a, tc.dtype_b), tc.dtype_out, tc_opt=2)
  ...
  def test_tensor_cores_padded_uops(self):
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      ...
      helper_tc_ensure_uops_and_opts_count(tc.dims[0]+pad, tc.dims[1]+pad, tc.dims[2]+pad,
                                           (tc.dtype_a, tc.dtype_b), tc.dtype_out, tc_opt=2, ensure_triggered=True)

      # check that TC is not triggered for TC_OPT<2
      helper_tc_ensure_uops_and_opts_count(tc.dims[0]+pad, tc.dims[1]+pad, tc.dims[2]+pad,
                                           (tc.dtype_a, tc.dtype_b), tc.dtype_out, tc_opt=1, ensure_triggered=False)
      helper_tc_ensure_uops_and_opts_count(tc.dims[0]+pad, tc.dims[1]+pad, tc.dims[2]+pad,
  ...
  def test_tensor_cores_multi_reduce(self):
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      if not is_dtype_supported(tc.dtype_a) or not is_dtype_supported(tc.dtype_b) or not is_dtype_supported(tc.dtype_out): continue
      if tc.dtype_a is dtypes.bfloat16: continue # <-- broken with numpy
      # this will be a M=G16, N=G32, M=G16, M=G16, K=R16, K=R16, K=R16 with 9 choices of TC MNK axes
      golden_result = None
      for axis in range(9):
        a = Tensor.rand(16, 16, 29, 29, dtype=tc.dtype_a).realize()
        b = Tensor.rand(32, 16, 16, 16, dtype=tc.dtype_b).realize()
  ...
  def test_tensor_cores_unroll_phi(self):
    ...
    x, y = Tensor.rand(128, 128, dtype=tc.dtype_a), Tensor.rand(128, 128, dtype=tc.dtype_b)
  ...
  def test_tensor_cores_unroll_casted_phi(self):
    tc = [tc for tc in Device[Device.DEFAULT].renderer.tensor_cores if tc.dtype_a != tc.dtype_out][0]
    x, y = Tensor.rand(128, 128, dtype=tc.dtype_a), Tensor.rand(128, 128, dtype=tc.dtype_b)
    r = x.matmul(y, dtype=tc.dtype_out)
    opts = [Opt(OptOps.UNROLL, 0, 4)]
    ast = helper_linearizer_opt(r, [opts], apply_tc=True, atol=3e-2, rtol=1e-3)
