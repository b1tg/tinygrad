...
def _skip_unsupported_tc_dtypes(dtype_in:tuple[DType, DType], dtype_out:DType):
  supported_dtypes = Device[Device.DEFAULT].renderer.supported_dtypes()
  if unsupported := [f"{name}={dtype}" for name,dtype in (("dtype_a", dtype_in[0]), ("dtype_b", dtype_in[1]), ("dtype_out", dtype_out))
                     if dtype not in supported_dtypes]:
    raise unittest.SkipTest(f"tensor core requires unsupported renderer dtype: {', '.join(unsupported)}")

def _tc_matmul(a:Tensor, b:Tensor, dtype:DType) -> Tensor:
  if a.dtype == b.dtype: return a.matmul(b, dtype=dtype)
  lhs = a.reshape(a.shape[0], a.shape[1], 1).expand(a.shape[0], a.shape[1], b.shape[1])
  rhs = b.reshape(1, b.shape[0], b.shape[1]).expand(a.shape[0], b.shape[0], b.shape[1])
  return lhs._apply_uop(lambda x,y: x.alu(Ops.MUL, y), rhs).sum(1, dtype=dtype).cast(dtype)
...
def helper_tc_ensure_uops_and_opts_count(N: int, M:int, K:int, dtype_in:tuple[DType, DType], dtype_out:DType,
                                         axis:int=0, tc_select:int=-1, tc_opt:int=0,
                                         ensure_triggered:bool=True):
  _skip_unsupported_tc_dtypes(dtype_in, dtype_out)
  a, b = _tc_rand(M, K, dtype=dtype_in[0]), _tc_rand(K, N, dtype=dtype_in[1])
  r = _tc_matmul(a, b, dtype_out)
...
def helper_tc_allclose(N:int, M:int, K:int, dtype_in:tuple[DType, DType], dtype_out:DType,
  ...
  elif not dtypes.is_float(dtype_in[0]): tc_atol, tc_rtol = 0, 0
  else: tc_atol, tc_rtol = 5e-3, 1e-4
  c = bufs[0].numpy().reshape((M,N))
  ref = (np_a.astype(np.int32) @ np_b.astype(np.int32)) if not dtypes.is_float(dtype_in[0]) else (np_a @ np_b)
...
class TestTensorCores(unittest.TestCase):
  ...
  def test_tensor_cores_nan(self):
    for tc in [tc for tc in Device[Device.DEFAULT].renderer.tensor_cores if all(dtypes.is_float(dt) for dt in tc.dtype_in)]:
      with self.subTest(tc=tc):
        _skip_unsupported_tc_dtypes(tc.dtype_in, tc.dtype_out)
        a = Tensor.full((tc.dims[1], tc.dims[2]), float("nan"), dtype=tc.dtype_in[0])
        b = Tensor.ones(tc.dims[2], tc.dims[0], dtype=tc.dtype_in[1])
        realized_ast, bufs = helper_realized_ast(_tc_matmul(a, b, tc.dtype_out))
    ...
    for i, tc in enumerate(Device[Device.DEFAULT].renderer.tensor_cores):
      with self.subTest(tc=tc):
        ...
        with Context(ALLOW_TF32=1, TC_SELECT=i, TC_OPT=2):
          a = _tc_rand(tc.dims[1]*8, tc.dims[2]*8, dtype=tc.dtype_in[0])
          b = _tc_rand(tc.dims[2]*8, tc.dims[0]*8, dtype=tc.dtype_in[1])
          ast = _tc_matmul(a, b, tc.dtype_out).schedule_linear().src[-1].src[0]
  ...
  def test_tensor_cores_upcast_shared_axis(self):
    # same operand shapes
    tc = next(tc for tc in Device[Device.DEFAULT].renderer.tensor_cores if tc.dtype_in[0] not in dtypes.fp8s)
    N, M, K = tc.dims
    a, b = Tensor.rand(3, M*2, K*2, dtype=tc.dtype_in[0]), Tensor.rand(3, K*2, N*2, dtype=tc.dtype_in[1])
  ...
  def test_tensor_cores_padto_warp(self):
    ...
    sche = Scheduler(Tensor.empty(64, 64, dtype=tc.dtype_in[0]).matmul(Tensor.empty(64, 64, dtype=tc.dtype_in[1]), dtype=tc.dtype_out)
  ...
  def test_tensor_cores_group_reduce(self):
    tc = next(tc for tc in Device[Device.DEFAULT].renderer.tensor_cores if tc.dtype_in[0] not in dtypes.fp8s)
    sche = Scheduler(Tensor.empty(16, 64, dtype=tc.dtype_in[0]).matmul(Tensor.empty(64, 16, dtype=tc.dtype_in[1]), dtype=tc.dtype_out)
                      .schedule_linear().src[-1].src[0], Device[Device.DEFAULT].renderer)
    sche.apply_opt(Opt(OptOps.TC, 0, (-1, 0, 1)))
    ...
    if AxisType.UNROLL in sche.axis_types:
      ...
      x, y = Tensor.rand(16, 64, dtype=tc.dtype_in[0]), Tensor.rand(64, 16, dtype=tc.dtype_in[1])
  ...
  def test_tensor_cores_failed_padto(self):
    ...
    sche = Scheduler(Tensor.empty(M//4, K, dtype=tc.dtype_in[0]).matmul(Tensor.empty(K, N+N//2, dtype=tc.dtype_in[1]), dtype=tc.dtype_out)
                     .schedule_linear().src[-1].src[0], Device[Device.DEFAULT].renderer)
  ...
  def test_tensor_cores_nested_reduce(self):
    tc = Device[Device.DEFAULT].renderer.tensor_cores[0]
    a = Tensor.empty(tc.dims[1]*2, tc.dims[2], dtype=tc.dtype_in[0])
    b = Tensor.empty(tc.dims[2], tc.dims[0], dtype=tc.dtype_in[1])
    ast = replace_opts(_tc_matmul(a, b, tc.dtype_out).sum(0).schedule_linear().src[-1].src[0], [Opt(OptOps.TC, 0, (-1, 0, 1))])
  ...
  def test_tensor_cores_contracted_m(self):
    ...
    a = Tensor.empty(m*2, k*2, dtype=tc.dtype_in[0])
    b, c = Tensor.empty(k*2, n*2, dtype=tc.dtype_in[1]), Tensor.empty(n*2, dtype=tc.dtype_out)
  ...
  def test_tensor_cores_codegen(self):
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      n, m, k = tc.dims
      a, b = _tc_rand(m, k, dtype=tc.dtype_in[0]), _tc_rand(k, n, dtype=tc.dtype_in[1])
      r = _tc_matmul(a, b, tc.dtype_out)
  ...
  @Context(ALLOW_TF32=1)
  @unittest.skipUnless(any(tc.dtype_in[0] in (dtypes.half, dtypes.float) for tc in Device[Device.DEFAULT].renderer.tensor_cores),
  ...
  def test_tensor_cores_padto_unroll(self):
    ...
    tc = next(tc for tc in Device[Device.DEFAULT].renderer.tensor_cores if tc.dtype_in[0] in (dtypes.half, dtypes.float))
    Tensor.manual_seed(3)
    a = Tensor.rand(tc.dims[1]*2+1, tc.dims[2]*3-1, dtype=tc.dtype_in[0]).realize()
    b = Tensor.rand(tc.dims[2]*3-1, tc.dims[0]*2+1, dtype=tc.dtype_in[1]).realize()
  ...
  def test_tensor_cores_padto_masked_operand(self):
    ...
    one = Tensor(1, dtype=tc.dtype_in[0])
  ...
  def test_tensor_cores_multi_reduce(self):
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      if tc.dtype_in[0] != tc.dtype_in[1] or tc.dtype_in[0] is dtypes.bfloat16: continue # <-- broken with numpy
  ...
  def test_tensor_cores_unroll_phi(self):
    ...
    tc = next(tc for tc in Device[Device.DEFAULT].renderer.tensor_cores if tc.dtype_in[0] not in dtypes.fp8s)
    x, y = Tensor.rand(16, 64, dtype=tc.dtype_in[0]).realize(), Tensor.rand(64, 16, dtype=tc.dtype_in[1]).realize()
  ...
  def test_tensor_cores_unroll_casted_phi(self):
    tc = [tc for tc in Device[Device.DEFAULT].renderer.tensor_cores if tc.dtype_in[0] != tc.dtype_out and tc.dtype_in[0] not in dtypes.fp8s][0]
    x, y = Tensor.rand(16, 64, dtype=tc.dtype_in[0]).realize(), Tensor.rand(64, 16, dtype=tc.dtype_in[1]).realize()
    opts = [Opt(OptOps.TC, 0, (-1, 0, 1)), Opt(OptOps.SPLIT, tc_reduce_axis(x.matmul(y, dtype=tc.dtype_out)), (2, AxisType.UNROLL))]
    r = x.matmul(y, dtype=tc.dtype_out)
    ast = helper_linearizer_opt(r, [opts[1:]], apply_tc=True, atol=3e-2, rtol=1e-3, check_default_opt=False)
  ...
  @unittest.skipUnless(Device[Device.DEFAULT].renderer.tensor_cores, "test requires tensor cores")
  @unittest.skipUnless(any(tc.dtype_in == (dtypes.half, dtypes.half) and tc.dtype_out == dtypes.half
                           for tc in Device[Device.DEFAULT].renderer.tensor_cores),
  ...
  def test_tc_shape_padded(self):
    tc = next(tc for tc in Device[Device.DEFAULT].renderer.tensor_cores if tc.dtype_in[0] in (dtypes.half, dtypes.float))
    Tensor.manual_seed(3)
    a, b = Tensor.rand(17, 23, dtype=tc.dtype_in[0]).realize(), Tensor.rand(23, 29, dtype=tc.dtype_in[1]).realize()
