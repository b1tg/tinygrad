...
def helper_tc_dtypes(tc): return tc.dtype_in
def helper_tc_dtype_supported(tc): return all(is_dtype_supported(dt) for dt in helper_tc_dtypes(tc)+(tc.dtype_out,))
...
def helper_tc_allclose(N:int, M:int, K:int, dtype_in:tuple[DType, DType], dtype_out:DType,
  ...
  r = a.matmul(b, dtype=dtype_out)
  if dtypes.bfloat16 in dtype_in: r = r.float()
  ...
  prg.exec(bufs)
  if dtypes.bfloat16 in dtype_in: tc_atol, tc_rtol = (1e-1, 2e-2) if dtype_out == dtypes.bfloat16 else (1e-2, 1e-2)
  elif dtypes.half in dtype_in: tc_atol, tc_rtol = 1e-2, 1e-3
...
class TestTensorCores(unittest.TestCase):
  ...
  def test_tensor_cores(self):
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      if not helper_tc_dtype_supported(tc): continue
      # for AMX, tc.dims[2] == 1 so reduceop is None thus tensor_cores are not triggered
      helper_tc_allclose(tc.dims[0], tc.dims[1], 2 if AMX else tc.dims[2], helper_tc_dtypes(tc), tc.dtype_out, axis=0, tc_opt=0)
  ...
  def test_tensor_cores_padded(self):
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      if not helper_tc_dtype_supported(tc): continue
      helper_tc_allclose(tc.dims[0]+(pad:=1), tc.dims[1]+pad, tc.dims[2]+pad, helper_tc_dtypes(tc), tc.dtype_out, tc_opt=2)
  ...
  def test_tensor_cores_padded_uops(self):
    for tc in Device[Device.DEFAULT].renderer.tensor_cores:
      if not helper_tc_dtype_supported(tc): continue
      pad = 1
      tc_dtypes = helper_tc_dtypes(tc)

      # check that TC is triggered for TC_OPT=2
      helper_tc_ensure_uops_and_opts_count(tc.dims[0]+pad, tc.dims[1]+pad, tc.dims[2]+pad,
                                           tc_dtypes, tc.dtype_out, tc_opt=2, ensure_triggered=True)

      # check that TC is not triggered for TC_OPT<2
      helper_tc_ensure_uops_and_opts_count(tc.dims[0]+pad, tc.dims[1]+pad, tc.dims[2]+pad,
                                           tc_dtypes, tc.dtype_out, tc_opt=1, ensure_triggered=False)
      helper_tc_ensure_uops_and_opts_count(tc.dims[0]+pad, tc.dims[1]+pad, tc.dims[2]+pad,
                                           tc_dtypes, tc.dtype_out, tc_opt=0, ensure_triggered=False)
