...
class PythonRemu:
  """Python RDNA3/RDNA4/CDNA4 emulator wrapper that matches the libremu.so interface."""
  ...
  arch: str = "rdna3"  # Architecture: rdna3, rdna4, cdna4
  ...
  def run_asm(self, lib: int, lib_sz: int, gx: int, gy: int, gz: int, lx: int, ly: int, lz: int, args_ptr: int) -> int:
    ...
    return run_asm(lib, lib_sz, gx, gy, gz, lx, ly, lz, args_ptr,
                   self.rsrc2, self.scratch_size, self.arch, self.user_data, self.valid_mem_ranges)
