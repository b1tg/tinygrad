...
class TestQ8Quantize(unittest.TestCase):
  ...
  def _check_quant_linear(self, ggml_type:int, scale_offset:int=0, scale_len:int=2, scale_vals=(0.01,),
                          check_decode=True, rtol_pre=2e-2, atol_pre=4e-1):
    if not amd_custom_kernels_supported(Tensor.empty(1).device): self.skipTest("RDNA3 required")
    from tinygrad.llm.kernels.amd import QUANT_FORMATS
    block_size, type_size, packed_dtype, _, _ = QUANT_FORMATS[ggml_type]
    rng = np.random.default_rng(42)
    in_features, out_features = 2048, 16
    blocks = out_features * in_features // block_size
    packed = rng.integers(0, 256, blocks * type_size, dtype=np.uint8)
    for i in range(blocks):
      packed[i*type_size+scale_offset : i*type_size+scale_offset+scale_len] = np.array(scale_vals, dtype=np.float16).view(np.uint8)
    raw = Tensor(np.pad(packed, (4, 0))).contiguous().realize()[4:]
    ...
    np.testing.assert_allclose(linear(Tensor(x_pre)).numpy(), x_pre @ weight.T, rtol=rtol_pre, atol=atol_pre)
    self.assertEqual(linear.ggml_type, ggml_type)
    self.assertEqual(linear.weight.dtype, packed_dtype)

  def test_q4_0_linear(self): self._check_quant_linear(2, rtol_pre=1e-2, atol_pre=1e-1)
  def test_q8_0_linear(self): self._check_quant_linear(8, rtol_pre=1e-2, atol_pre=1e-1)
  def test_q5_k_linear(self): self._check_quant_linear(13, scale_offset=0, scale_len=4, scale_vals=(0.01, 0.002), rtol_pre=1e-2, atol_pre=1e-1)
  def test_q6_k_wmma_prefill(self): self._check_quant_linear(14, scale_offset=208, check_decode=False)
  def test_iq3_xxs_linear(self): self._check_quant_linear(18)
  def test_iq2_xs_linear(self): self._check_quant_linear(17)
