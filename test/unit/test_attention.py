...
class TestGatedDeltaNetBlock(unittest.TestCase):
  def test_gated_delta_rectangular_state_and_key_decay(self):
    ...
    for t in range(3):
      previous = expected_state * alpha[:, :, t, None, :]
      delta = (v[:, :, t] - (previous*k[:, :, t, None]).sum(-1)) * beta[:, :, t, None]
      expected_state = previous + delta[..., None]*k[:, :, t, None, :]
      expected_out[:, :, t] = (expected_state*q[:, :, t, None]).sum(-1)
  ...
  def test_kda_channel_decay(self):
    ...
    # State is (B,H,V,K): KDA forget gates scale keys, not values.
    np.testing.assert_allclose(block.recurrent_state.numpy(), initial_state.numpy() * alpha[:, None, :], rtol=1e-5, atol=1e-5)

  def test_kda_normalization_small_inputs(self):
    config = self._make_config(ssm=SSMConfig(conv_kernel=2, state_size=4, group_count=1, time_step_rank=1,
                                           inner_size=4, kda=True, norm_eps=1e-6))
    for magnitude in (0.0, 1e-5, 0.1):
      with self.subTest(magnitude=magnitude):
        block = GatedDeltaNetBlock(config, config.ssm)
        block.attn_qkv.weight = Tensor.full((12, config.dim), magnitude)
        block.ssm_beta.weight = Tensor.zeros(1, config.dim)
        block.ssm_conv1d['weight'] = Tensor.ones(12, 2)
        x = Tensor.ones(1, 1, config.dim)
        block._init_state(x)
        block._attention(x, 0).realize()
        projected = np.float32(config.dim * magnitude)
        conv = projected / (1 + np.exp(-projected))
        key = conv / np.sqrt(4 * conv**2 + 1e-6)
        np.testing.assert_allclose(block.recurrent_state.numpy(), np.full((1,1,4,4), 0.5 * conv * key), rtol=1e-4, atol=1e-10)
