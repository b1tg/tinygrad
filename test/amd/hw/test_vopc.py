...
class TestCmpClass(unittest.TestCase):
  ...
  def test_v_cmp_lg_f32_nan(self):
    """v_cmp_lg_f32 is ordered not-equal (<>): NaN <> x must be False per IEEE 754."""
    quiet_nan = 0x7fc00000
    ...
    st = run_program(instructions, n_lanes=1)
    self.assertEqual(st.vcc & 1, 0, "v_cmp_lg_f32(NaN, 1.0) should be 0 (ordered comparison returns False for NaN)")
