...
class TestFp8sConversions(unittest.TestCase):
  ...
  def test_float_to_fp8e4m3_extreme_values(self):
    np.testing.assert_equal(float_to_fp8(FP8E4M3_MAX, dtypes.fp8e4m3), 126)          # max_norm → 0x7E (max_norm)
    np.testing.assert_equal(float_to_fp8(FP8E4M3_MAX*1.01, dtypes.fp8e4m3), 126)     # overflow → 0x7E (saturate to max_norm)
    np.testing.assert_equal(float_to_fp8(math.inf, dtypes.fp8e4m3), 127)             # +inf → 0x7F (NaN, e4m3 has no inf)
    np.testing.assert_equal(float_to_fp8(-FP8E4M3_MAX, dtypes.fp8e4m3), 254)         # -max_norm → 0xFE
    np.testing.assert_equal(float_to_fp8(-FP8E4M3_MAX*1.01, dtypes.fp8e4m3), 254)    # -overflow → 0xFE (saturate to -max_norm)
    np.testing.assert_equal(float_to_fp8(-math.inf, dtypes.fp8e4m3), 255)            # -inf → 0xFF (NaN)
    np.testing.assert_equal(float_to_fp8(math.nan, dtypes.fp8e4m3), 127)             # nan → 0x7F (positive NaN)
    np.testing.assert_equal(float_to_fp8(-math.nan, dtypes.fp8e4m3), 255)            # -nan → 0xFF (negative NaN)
  ...
  def test_float_to_fp8e5m2_extreme_values(self):
    np.testing.assert_equal(float_to_fp8(FP8E5M2_MAX, dtypes.fp8e5m2), 123)          # max_norm → 0x7B
    np.testing.assert_equal(float_to_fp8(FP8E5M2_MAX*1.01, dtypes.fp8e5m2), 123)     # overflow → 0x7B (saturate to max_norm)
    np.testing.assert_equal(float_to_fp8(math.inf, dtypes.fp8e5m2), 124)             # +inf → 0x7C
    np.testing.assert_equal(float_to_fp8(-FP8E5M2_MAX, dtypes.fp8e5m2), 251)         # -max_norm → 0xFB
    np.testing.assert_equal(float_to_fp8(-FP8E5M2_MAX*1.01, dtypes.fp8e5m2), 251)    # -overflow → 0xFB (saturate to -max_norm)
    np.testing.assert_equal(float_to_fp8(-math.inf, dtypes.fp8e5m2), 252)            # -inf → 0xFC
    np.testing.assert_equal(float_to_fp8(math.nan, dtypes.fp8e5m2), 126)             # nan → 0x7E (positive NaN)
    np.testing.assert_equal(float_to_fp8(-math.nan, dtypes.fp8e5m2), 254)            # -nan → 0xFE (negative NaN)
  ...
  def test_float_to_fp8e4m3fnuz_extreme_values(self):
    # fnuz: no inf, no -0; 0x80 is the only NaN; overflow saturates to max_norm
    np.testing.assert_equal(float_to_fp8(FP8E4M3FNUZ_MAX, dtypes.fp8e4m3fnuz), 0x7f)       # max_norm → 0x7F
    np.testing.assert_equal(float_to_fp8(FP8E4M3FNUZ_MAX*1.01, dtypes.fp8e4m3fnuz), 0x7f)  # overflow → 0x7F (saturate to max_norm)
    np.testing.assert_equal(float_to_fp8(math.inf, dtypes.fp8e4m3fnuz), 0x80)              # +inf → 0x80 (NaN)
    np.testing.assert_equal(float_to_fp8(-FP8E4M3FNUZ_MAX, dtypes.fp8e4m3fnuz), 0xff)      # -max_norm → 0xFF
    np.testing.assert_equal(float_to_fp8(-FP8E4M3FNUZ_MAX*1.01, dtypes.fp8e4m3fnuz), 0xff) # -overflow → 0xFF (saturate to -max_norm)
    np.testing.assert_equal(float_to_fp8(-math.inf, dtypes.fp8e4m3fnuz), 0x80)             # -inf → 0x80 (NaN)
    np.testing.assert_equal(float_to_fp8(math.nan, dtypes.fp8e4m3fnuz), 0x80)              # nan → 0x80 (NaN)
    np.testing.assert_equal(float_to_fp8(0.0, dtypes.fp8e4m3fnuz), 0x00)                   # +0 → 0x00 (zero)
    np.testing.assert_equal(float_to_fp8(-0.0, dtypes.fp8e4m3fnuz), 0x00)                  # -0 → 0x00 (no negative zero in fnuz)
  ...
  def test_float_to_fp8e5m2fnuz_extreme_values(self):
    # fnuz: no inf, no -0; 0x80 is the only NaN; overflow saturates to max_norm
    np.testing.assert_equal(float_to_fp8(FP8E5M2FNUZ_MAX, dtypes.fp8e5m2fnuz), 0x7f)       # max_norm → 0x7F
    np.testing.assert_equal(float_to_fp8(FP8E5M2FNUZ_MAX*1.01, dtypes.fp8e5m2fnuz), 0x7f)  # overflow → 0x7F (saturate to max_norm)
    np.testing.assert_equal(float_to_fp8(math.inf, dtypes.fp8e5m2fnuz), 0x80)              # +inf → 0x80 (NaN)
    np.testing.assert_equal(float_to_fp8(-FP8E5M2FNUZ_MAX, dtypes.fp8e5m2fnuz), 0xff)      # -max_norm → 0xFF
    np.testing.assert_equal(float_to_fp8(-FP8E5M2FNUZ_MAX*1.01, dtypes.fp8e5m2fnuz), 0xff) # -overflow → 0xFF (saturate to -max_norm)
    np.testing.assert_equal(float_to_fp8(-math.inf, dtypes.fp8e5m2fnuz), 0x80)             # -inf → 0x80 (NaN)
    np.testing.assert_equal(float_to_fp8(math.nan, dtypes.fp8e5m2fnuz), 0x80)              # nan → 0x80 (NaN)
    np.testing.assert_equal(float_to_fp8(0.0, dtypes.fp8e5m2fnuz), 0x00)                   # +0 → 0x00 (zero)
    np.testing.assert_equal(float_to_fp8(-0.0, dtypes.fp8e5m2fnuz), 0x00)                  # -0 → 0x00 (no negative zero in fnuz)
