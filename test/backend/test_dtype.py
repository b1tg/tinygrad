...
class TestFp8sConversions(unittest.TestCase):
  ...
  def test_float_to_fp8e4m3fnuz_extreme_values(self):
    ...
    np.testing.assert_equal(float_to_fp8(FP8E4M3FNUZ_MAX*1.01, dtypes.fp8e4m3fnuz), 0x7f)
    np.testing.assert_equal(float_to_fp8(math.inf, dtypes.fp8e4m3fnuz), 0x80)
    np.testing.assert_equal(float_to_fp8(-FP8E4M3FNUZ_MAX, dtypes.fp8e4m3fnuz), 0xff)
    np.testing.assert_equal(float_to_fp8(-FP8E4M3FNUZ_MAX*1.01, dtypes.fp8e4m3fnuz), 0xff)
    np.testing.assert_equal(float_to_fp8(-math.inf, dtypes.fp8e4m3fnuz), 0x80)
    np.testing.assert_equal(float_to_fp8(math.nan, dtypes.fp8e4m3fnuz), 0x80)
    np.testing.assert_equal(float_to_fp8(0.0, dtypes.fp8e4m3fnuz), 0x00)
    np.testing.assert_equal(float_to_fp8(-0.0, dtypes.fp8e4m3fnuz), 0x00)
  ...
  def test_float_to_fp8e5m2fnuz_extreme_values(self):
    np.testing.assert_equal(float_to_fp8(FP8E5M2FNUZ_MAX, dtypes.fp8e5m2fnuz), 0x7f)
    np.testing.assert_equal(float_to_fp8(FP8E5M2FNUZ_MAX*1.01, dtypes.fp8e5m2fnuz), 0x7f)
    np.testing.assert_equal(float_to_fp8(math.inf, dtypes.fp8e5m2fnuz), 0x80)
    np.testing.assert_equal(float_to_fp8(-FP8E5M2FNUZ_MAX, dtypes.fp8e5m2fnuz), 0xff)
    np.testing.assert_equal(float_to_fp8(-FP8E5M2FNUZ_MAX*1.01, dtypes.fp8e5m2fnuz), 0xff)
    np.testing.assert_equal(float_to_fp8(-math.inf, dtypes.fp8e5m2fnuz), 0x80)
    np.testing.assert_equal(float_to_fp8(math.nan, dtypes.fp8e5m2fnuz), 0x80)
    np.testing.assert_equal(float_to_fp8(0.0, dtypes.fp8e5m2fnuz), 0x00)
    np.testing.assert_equal(float_to_fp8(-0.0, dtypes.fp8e5m2fnuz), 0x00)
