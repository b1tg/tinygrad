...
def _test_cast(a:Tensor, target_dtype:DType):
  # print(f"_test_cast: {a.dtype} => {target_dtype}, {_to_torch_dtype(target_dtype)=}")
  # print(f"  {a.tolist()}")
...
def _test_bitcast(a:Tensor, target_dtype:DType, target=None):
  # print(f"_test_bitcast: {a.dtype} => {target_dtype}, {_to_torch_storage_type(a.dtype)=}, {_to_torch_dtype(target_dtype)=}")
  # print(f"  {a.tolist()}")
...
class TestBitCast(unittest.TestCase):
  ...
  # @given(strat.sampled_from([dtypes.fp8e4m3]), strat.sampled_from([dtypes.char]))
  # @given(strat.sampled_from([dtypes.char]), strat.sampled_from([dtypes.fp8e4m3]))
  def test_shape_change_bitcast(self, dt1, dt2):
    ...
    # TODO: RuntimeError: value cannot be converted to type uint8 without overflow
    # assume(not (getenv("PYTHON") and dt1 in dtypes.fp8s and dt2 in { dtypes.int8, dtypes.uint8 }))
    data = rand_for_dtype(dt1, 32).reshape(2, 2, 8)
    # print(f" {dt1}=>{dt2}  data: {data=}")
    # if dt1 in dtypes.fp8s:
      # data = rand_for_dtype(dt1, 32).astype(np.uint8).reshape(2, 2, 8)
      # data = np.random.uniform(-10, 10, size=32).astype(np.uint8).reshape(2, 2, 8)
    ...
    if dt2 in dtypes.fp8s:
      ...
      # expected = list(map(lambda x: fp8_to_float(x, dt2), expected))
    # print(f"test_shape_change_bitcast: {dt1=}, {_to_torch_storage_type(dt1)=} {dt2=}, {_to_torch_dtype(dt2)=} {expected=}")
